...
#working with tweets using code on web, this is the basic code, the
#Location and name of the sql lite might have to be adjusted
#when used again.. it creates a new table named tweet
#after it runs, might want to change that to not have
#multiples of same name in sqlite data base..pretty neat though!
#i downloaded my tweets..they go back a ways
#http://www.curiositybits.com/new-page-3/
#when exporting to csv -- Make sure to ADD THE CSV EXTENSION NAME!!!!
#also, save the csv to atom, you can see the content there, sometimes if
#you open in excel, it shows up as empty! don't know why. frustrating!!
#also it will only collect the first 200 tweets, you can make the template
#down to 200 for one person for example, not sure if that helps..
...
t = Twython(app_key='7TVbwixTSsE2JQDl2cUaR4LC5',
    app_secret='RUhXft6Wxmz23Ml50TeCzvatVbhAMFonlukLhTwelwewUfCAIe',
    oauth_token='369694932-SNZDvI1qt88Nvy3XEZvBp9r7UwSOLL3lXjQbObA9',
    oauth_token_secret='3WzWY6gsIExFZWIDCRiOve1ugp9kkjOc5KWxObVkayzNU')


Base = declarative_base()
class TWEET(Base):
    __tablename__ = 'tweets1' # This is table name. it can be any name, it will be created as this runs under accounts
...
class ACCOUNT(Base):    #ie accounts is the sqlite table i create, the other name above will be created as this runs
...
class Scrape:
    def __init__(self):
        engine = sqlalchemy.create_engine("sqlite:///tweets.sqlite", echo=False)  # different DB name here
